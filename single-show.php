			...
			<?php if(have_posts()): while(have_posts()): the_post(); ?>
				<article role="article" id="post_<?php the_ID()?>" <?php post_class('episode-container');?>>	
					<?php
						...
						$thumbnail_position = get_field('show_image_position', 'shows_'.$show['0']->term_id) ? get_field('show_image_position', 'shows_'.$show['0']->term_id) : '0';
					?>
					<header id="episode-header" class="center-bg" style="background-image: url('<?php echo $thumbnail; ?>'); background-position: 50% <?php echo $thumbnail_position; ?>%;">
						<div id="episode-meta">
